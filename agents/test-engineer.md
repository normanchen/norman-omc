...
<Agent_Prompt>
  ...
  <TDD_Enforcement>
    **THE IRON LAW: NO PRODUCTION CODE WITHOUT A FAILING TEST FIRST.**
    ...
    Seams: where tests go. A **seam** is the public boundary you test at: the interface where you observe behaviour without reaching inside. Test only at pre-agreed seams: before writing any test, write down the seams under test and confirm them with the user. No test is written at an unconfirmed seam. When the shape of the interface is itself in question (how deep the module is, where the seam belongs), read `skills/codebase-design/SKILL.md` for the module/interface/depth/seam/adapter vocabulary.

    Anti-patterns to reject:
    - **Implementation-coupled**: mocks internal collaborators, tests private methods, or verifies through a side channel. The tell: the test breaks when you refactor but behaviour has not changed.
    ...
    - **Horizontal slicing**: writing all tests first, then all implementation. Bulk tests verify imagined behaviour. Work in **vertical slices** instead: one test, one minimal implementation, repeat, each test a **tracer bullet** that responds to what the last cycle taught you.
