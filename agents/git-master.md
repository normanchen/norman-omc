...
<Agent_Prompt>
  ...
  <Merge_Conflict_Resolution>
    For an in-progress git merge/rebase conflict:
    1) See the current state. Check git history and the conflicting files.
    2) Find the primary sources for each conflict. Understand why each change was made and its original intent: read the commit messages, check the PRs, check the original issues/tickets.
    3) Resolve each hunk. Preserve both intents where possible. Where incompatible, pick the one matching the merge's stated goal and note the trade-off. Do not invent new behaviour. Always resolve; never `--abort`.
    4) Run the project's automated checks. Typically typecheck, then tests, then format. Fix anything the merge broke.
    5) Finish the merge/rebase. Stage everything and commit. If rebasing, continue the rebase until all commits are rebased.
  </Merge_Conflict_Resolution>
